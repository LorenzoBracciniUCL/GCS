...
def equation_dot_sigma_jk(t, y, H_J, H_K, n_modes):
    """
    ...
    # Compute derivative
    sigma_jk_dot = 0.5 * (Omega @ (H_J + H_K) @ sigma_jk - sigma_jk @ (H_J + H_K) @ Omega ) - 1j * (sigma_jk@(H_J - H_K) @ sigma_jk + Omega @ (H_J - H_K) @ Omega)

    return  sigma_jk_dot.flatten()

def Compute_sigma_JK_Unitary_Gaussian_General_Numerical(n_modes, sigma_0, H_J, H_K, t_array):
    ...
    sigma_jk_sol = solve_ivp(equation_dot_sigma_jk, t_span, y0, t_eval=t_array, args=(H_J, H_K, n_modes),method='DOP853')
...
def equation_dot_r_jk(t, y, sigma_jk_interp, H_j, H_k, r_j, r_k, n_modes):
    """
...
def Compute_R_JK_Unitary_Gaussian_General_Numerical(n_modes,r_0, sigma_JK_sol, H_j, H_k, r_j, r_k, t_array):
    ...
    # Solve for r_jk
    sol_r = solve_ivp(equation_dot_r_jk, t_span, y0, t_eval=t_array, args=(sigma_JK_interp, H_j, H_k, r_j, r_k, n_modes),method='DOP853')
...
def Compute_R_JK_0_Unitary_Gaussian_General_Numerical(n_modes, r_JK_sol, sigma_JK_sol,  H_j, H_k, r_j, r_k, J_labels, K_labels, H_q_0_array, rho_q_0_JK, t_array):
    ...
    sol_r_0 = solve_ivp(equation_dot_r_jk_0, t_span, y0, t_eval=t_array, 
                        args=(sigma_JK_interp,r_JK_interp, H_j, H_k, r_j, r_k, J_labels, K_labels, H_q_0_array, n_modes), method='DOP853')
    # Extract solutioon
